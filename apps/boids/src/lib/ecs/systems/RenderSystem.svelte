<script lang="ts">
	...
	import { BoidScoutGroupComponent, ColorComponent, PositionComponent } from '../components.svelte';
	...
	updater.add(() => {
		...
		entities.forEach((entity) => {
			...
			const group = entity.componentsByConstructor.get(BoidScoutGroupComponent) as BoidScoutGroupComponent | undefined;
			if (!position) return;
			canvas.context.lineCap = 'round';
			canvas.context.beginPath();
			canvas.context.strokeStyle = !group ? '#FFF' : group.group == 1 ? '#33D' : '#D33';
